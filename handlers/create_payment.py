...
from keyboards import payment_methods_keyboard, purchase_unlimited_keyboard, payment_keyboard, social_check_keyboard
...
@dp.callback_query_handler(text='method_palka')
async def palka_payment(callback: types.CallbackQuery, state: FSMContext):
    text = """Payment Method: PayPal
Cost: 25.00 💵USD
Validity: indefinitely 

when paying  is obligatory to pay to «friends&famaly / gift» otherwise the payment will be canceled!

After payment, the bot will immediately send you your product"""
    await callback.message.edit_text(text, reply_markup=payment_keyboard.make_paypal())
...
async def cancel(callback: types.CallbackQuery):
    if cheack_lang(callback.message.chat.id) == 'ru':
